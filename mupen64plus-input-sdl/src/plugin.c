...
extern void sdl2_loader_init();
extern void sdl2_loader_deinit();
static int g_report_isRumbling = 0;
              ...
              if (dwAddress == PAK_IO_RUMBLE && *Data)
                ...
                if(dwAddress == PAK_IO_RUMBLE && controller[Control].event_joystick) {
                    if (*Data) {
                        ...
                        g_report_isRumbling = 1;
                    } else {
                        SDL_HapticRumbleStop(controller[Control].event_joystick);
                        g_report_isRumbling = 0;
                    }
                }
                else if (dwAddress == PAK_IO_RUMBLE) {
                    if (*Data) {
                        g_report_isRumbling = 1;
                    } else {
                        g_report_isRumbling = 0;
    ...
    *Keys = controller[Control].buttons;
    ...
    // if (controller[Control].event_joystick) {
        ...
        if (controller[Control].buttons.Value & button_bits[14]) {
            ...
            if (controller[Control].event_joystick) SDL_HapticRumblePlay(controller[Control].event_joystick, 0.5, 500);
            g_report_isRumbling = 1;
        }
        if (controller[Control].buttons.Value & button_bits[15]) {
            ...
            controller[Control].control->Plugin = PLUGIN_NONE;// remove old pack
            if (controller[Control].event_joystick)  SDL_HapticRumblePlay(controller[Control].event_joystick, 1, 500);
            g_report_isRumbling = 1;
        }
        // handle inserting new pack if the time has arrived
        if (SwitchPackTime[Control] != 0 && (SDL_GetTicks() - SwitchPackTime[Control]) >= 1000)
        {
            if (controller[Control].event_joystick) SDL_HapticRumbleStop(controller[Control].event_joystick);
            g_report_isRumbling = 0;
...
static void InitiateRumble(int cntrl)
{
    ...
    if (!controller[cntrl].event_joystick) {
        DebugMessage(M64MSG_WARNING, "Couldn't open rumble support for joystick #%i: %s", cntrl + 1, SDL_GetError());
        return;
    }
...
EXPORT int CALL Report_IsRumbling(void) 
{
    return g_report_isRumbling;
}
